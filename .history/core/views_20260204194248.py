...
from .models import MissingPerson, FoundPerson, User
...
def search(request):
    ...
    # Start with verified cases only
    results = MissingPerson.objects.filter(status='verified')
...
def dashboard(request):
    """Admin/Police dashboard for case management"""
    if not request.user.can_verify_cases():
        messages.error(request, 'Access denied. Insufficient permissions.')
        return redirect('core:home')
    # Require verified status as well
    if not request.user.is_verified:
        messages.error(request, 'Access denied. Your account is pending verification.')
        return redirect('core:home')
    
    # Get statistics
    pending_cases = MissingPerson.objects.filter(status='pending').count()
    verified_cases = MissingPerson.objects.filter(status='verified').count()
    closed_cases = MissingPerson.objects.filter(status='closed').count()
    
    # Get recent pending cases
    recent_pending = MissingPerson.objects.filter(status='pending').order_by('-created_at')[:10]
...
def report_found_view(request):
    ...
    if not request.user.is_ngo() and not request.user.role == User.Roles.VOLUNTEER:
        messages.error(request, 'Only NGO workers and volunteers can report found persons.')
        return redirect('core:home')
    # Require verified status for NGO/Volunteer
    if not request.user.is_verified:
        messages.error(request, 'Your account is pending verification. You can report found persons after verification.')
        return redirect('core:home')
